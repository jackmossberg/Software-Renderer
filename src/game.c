  ...
  for (int i = 0; i < MAX_TRI_COUNT; i++) {
    ...
    model->tris[i].v3[2] *= model->scale[2];
      ...
      for (int i = 0; i < MAX_TRI_COUNT; ++i) {
            ...
            if (x1 == 0.0f && y1 == 0.0f && z1 == 0.0f && x2 == 0.0f && y2 == 0.0f && z2 == 0.0f && x3 == 0.0f && y3 == 0.0f && z3 == 0.0f)
                  continue;
...
void update_graphics(SDL_display *display) {
  ...
  main_camera.position[0] = ((float)mouseX / (float)display->window_width) * 10.0f - 5.0f;
  main_camera.position[1] = ((float)mouseY / (float)display->window_height) * 10.0f - 5.0f;
